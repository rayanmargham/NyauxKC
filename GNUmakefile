...
QEMUFLAGS := -m 2G -serial stdio -device virtio-gpu
...
run: run-$(ARCH)
.PHONY: run-kvm
run-kvm: run-$(ARCH)-kvm
.PHONY: run-debug
run-debug: run-$(ARCH)-debug
.PHONY: run-hdd
...
run-x86_64-kvm: edk2-ovmf-bins $(IMAGE_NAME).iso
	qemu-system-$(ARCH) \
		--enable-kvm \
		-cpu host \
		-M q35 \
		...
		-cdrom $(IMAGE_NAME).iso \
		$(QEMUFLAGS)
run-x86_64-debug: edk2-ovmf-bins $(IMAGE_NAME).iso
	qemu-system-$(ARCH) \
		-M q35 \
		-s \
		-S \
