...
void bubblesort(int length) {
  for (int i = 0; i < length; i++) {
    for (int j = 0; j < (length - i - 1); j++) {
      if (symbolarray->array[j].function_address <
          symbolarray->array[j + 1].function_address) {
...
void get_symbols() {
  ...
  if (strtab && symtab) {
    ...
    nyauxsymbol *array =
        (nyauxsymbol *)(kmalloc(sizeof(nyauxsymbol) * amou_of_sym));
    memset(array, 0, sizeof(nyauxsymbol) * amou_of_sym);
    uint64_t count = 0;
    for (uint64_t i = 1; i != amou_of_sym; i++) {
      Elf64_Sym *symbo =
          (Elf64_Sym *)(get_kerneL_address() + symtab->sh_offset +
                        (i * symtab->sh_entsize));
      if (symbo->st_value == 0)
        continue;
