	...
	if (rw == 0) {
		...
		restart1:
			...
			if (res == 0 && ((hnd->flags & O_NONBLOCK) ||
							 (tty->termi.c_cc[VMIN] == 0 &&
							  tty->termi.c_cc[VTIME] == 0)) == false) {
			...
			if ((char)val == 0x04  && tty->termi.c_lflag & ICANON) {
spinlock_unlock(&tty->rxlock);
				return 0; // no more program :)
				break;
			}
			((char *)buffer)[i] = (char)val;
		if (tty->termi.c_lflag & ECHO) {
			sprintf("echoing ur mom\r\n");
			char e[2] = {val, '\0'};
			flanterm_write(get_fctx(),e, sizeof(e));
		}
			spinlock_unlock(&tty->rxlock);
		}
...
static enum ttyps2state curstate = NORMAL;
void ps2_put_input() {
	struct vnode *lah = NULL;
	// just hope the underlying device recongizes that the kernel is asking this
	// shit
			...
			if (curtty->rx) {
				...
				if (yes.revents & POLLIN) {
					...
					if (pac.flags == PRESSED) {
						...
						} else if (curstate == SHIFT) {
							pac.ascii = ps2_scancode_to_uppercase(pac.keycode);
						}
						switch (pac.keycode) {
					...
					no:
					} else if (pac.flags == EXTENDEDKEYPRESSED) {
						sprintf_log(STATUSOK, "WE GOT SHIT\r\n");

					} else if (pac.flags == RELEASED) {
						switch (pac.keycode) {
						case PS2_SC_LEFT_SHIFT:
							curstate = NORMAL;
							break;
...
void devtty_init(struct vfs *curvfs) {
	...
	newtty->termi.c_cc[VINTR] = 0x03;
	newtty->termi.c_cc[VQUIT] = 0x1c;
	newtty->termi.c_cc[VERASE] = '\b';
	newtty->termi.c_cc[VKILL] = 0x15;
	newtty->termi.c_cc[VEOF] = 0x04;
	newtty->termi.c_cc[VSTART] = 0x11;
	newtty->termi.c_cc[VSTOP] = 0x13;
	newtty->termi.c_cc[VSUSP] = 0x1a;
	...
	hnd2->node = res;
	hnd3->node = res;
	char *optionchk[3] = {"serial_input", "serial_ttyinput", "serial.input"};
	...
	if (obj == NULL) {
		kprintf_log(STATUSFAIL,
					"not using serial input due to cmdline option...\r\n");
		goto mrrp;
	}
	if (obj->type == Bool) {
