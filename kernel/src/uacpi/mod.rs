...
use nyaux_uacpi_bindings::{
    UACPI_LOG_DEBUG, UACPI_LOG_ERROR, UACPI_LOG_INFO, UACPI_LOG_TRACE, UACPI_LOG_WARN,
    UACPI_STATUS_OK, UACPI_STATUS_UNIMPLEMENTED, uacpi_bool, uacpi_char, uacpi_cpu_flags,
    uacpi_firmware_request, uacpi_handle, uacpi_init_level, uacpi_initialize,
    uacpi_interrupt_handler, uacpi_io_addr, uacpi_log_level, uacpi_namespace_initialize,
    uacpi_namespace_load, uacpi_pci_address, uacpi_phys_addr, uacpi_setup_early_table_access,
    uacpi_size, uacpi_status, uacpi_status_to_string, uacpi_thread_id, uacpi_u8, uacpi_u16,
    uacpi_u32, uacpi_u64, uacpi_work_handler, uacpi_work_type,
};

use crate::{
    HHDM_REQUEST, RSDP_REQUEST, align_down,
    arch::{Arch, Processor},
    early_init_pagemap,
    memory::{
        slab::{slab_alloc, slab_dealloc},
        vmm::{VMMFlags, kermap, max_hhdm_phy},
    },
    pci::{
        pci_init, pci_read_byte, pci_read_dword, pci_read_word, pci_write_byte, pci_write_dword,
        pci_write_word,
    },
    print, println,
    util::SpinLock,
...
unsafe extern "C" fn uacpi_kernel_map(
    ...
    if addr < (*max_hhdm_phy.get().unwrap()) as u64 {
        ...
        early_init_pagemap!()
            .vmm_alloc_with_phys(len, VMMFlags::WRITE, ve)
            .unwrap()
            .cast()
...
unsafe extern "C" fn uacpi_kernel_log(arg1: uacpi_log_level, arg2: *const uacpi_char) {
    match arg1 {
        ...
        UACPI_LOG_WARN => {
            print!(
                "(warning) {}",
                CStr::from_ptr(arg2.cast()).to_str().unwrap()
            )
        }
        _ => {}
    }
...
unsafe extern "C" fn uacpi_kernel_pci_device_open(
    ...
    unsafe { out_handle.write(Box::into_raw(Box::new(address)) as *mut c_void) }
    UACPI_STATUS_OK
}
#[unsafe(no_mangle)]
...
unsafe extern "C" fn uacpi_kernel_io_map(
    ...
    let h = Box::new(io_range { base, len });
...
unsafe extern "C" fn uacpi_kernel_io_read16(
    ...
    let a: &io_range = unsafe { &*arg1.cast::<io_range>() };
    unsafe {
        out_value.write_volatile(Processor::raw_io_in(
            (a.base as u64)
                + offset as u64
                + if cfg!(target_arch = "riscv64") {
                    HHDM_REQUEST.response().unwrap().offset as u64
                } else {
                    0
                },
            2,
...
unsafe extern "C" fn uacpi_kernel_lock_spinlock(arg1: uacpi_handle) -> uacpi_cpu_flags {
    let ar: &SpinLock = unsafe { &*arg1.cast::<SpinLock>() };
    ar.lock();
    return 0;
}
#[unsafe(no_mangle)]
...
pub fn check_ustatus(s: u32) -> Result<(), &'static str> {
    let m = unsafe { CStr::from_ptr(uacpi_status_to_string(s)).to_str() }.unwrap();
    println!("{}", m);
    if s != 0 {
        return Err(m);
    }
