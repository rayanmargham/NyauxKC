...
result region_setup(pagemap* map, uint64_t hddm_in_pages)
{
	...
	VMMRegion* kernel = create_region(kernel_address.response->virtual_base, kernel_size);
	VMMRegion* hhdm = create_region(hhdm_request.response->offset, hddm_in_pages * 4096);
...
void* kvmm_region_alloc(uint64_t amount, uint64_t flags)
{
	...
	while (cur != NULL)
	{
		...
		{
			VMMRegion* new = create_region((prev->base + prev->length), align_up(amount, 4096));
