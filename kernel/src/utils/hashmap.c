										   ...
										   void (*elfree)(void* item), void* udata)
{
	...
	map->growat = fixedpt_toint(fixedpt_xmul(fixedpt_fromint(map->nbuckets),
											 fixedpt_xdiv(fixedpt_fromint(map->loadfactor), fixedpt_fromint(100))));
...
NYAUX_NO_UBSAN bool hashmap_iter(struct hashmap* map, size_t* i, void** item)
{
