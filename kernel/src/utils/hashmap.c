...
#define GROW_AT                                                                \
  (fixedpt_xdiv(fixedpt_fromint(60), fixedpt_fromint(100))) /* 60% */
#define SHRINK_AT                                                              \
  (fixedpt_xdiv(fixedpt_fromint(10), fixedpt_fromint(100))) /* 60% */

#define percentage_to_fixedpt(percent)                                         \
  (fixedpt_xdiv(fixedpt_fromint(percent), fixedpt_fromint(100)))

#ifndef HASHMAP_LOAD_FACTOR
#define HASHMAP_LOAD_FACTOR GROW_AT
#endif

// Fixed allocator functions now use sized frees.
static void *(*__malloc)(size_t) = NULL;
static void *(*__realloc)(void *, size_t) = NULL;
// __free now takes an extra parameter (the allocation size)
static void (*__free)(void *, size_t) = NULL;
...
// only once at startup and prior to calling hashmap_new().
// The free function now must have the signature: void free(void*, size_t)
void hashmap_set_allocator(void *(*malloc)(size_t),
...
struct hashmap {
  ...
  void (*free)(void *, size_t); // sized free function
  size_t elsize;
  size_t cap;
  uint64_t seed0;
  uint64_t seed1;
  ...
  // elfree now receives the size of the element
  void (*elfree)(void *item, size_t size);
  void *udata;
  size_t bucketsz;
  size_t nbuckets;
  size_t count;
...
// helper: compute the total size allocated for the hashmap object itself.
static size_t hashmap_alloc_size(struct hashmap *map) {
  return sizeof(struct hashmap) + map->bucketsz * 2;
}
...
static fixedptd clamp_load_factor(fixedptd factor, fixedptd default_factor) {
  ...
  return (factor < percentage_to_fixedpt(50)) ? percentage_to_fixedpt(50)
         : factor > percentage_to_fixedpt(95) ? percentage_to_fixedpt(95)
                                              : factor;
}
...
// The free function pointer (_free) must now be of the type accepting a size.
struct hashmap *hashmap_new_with_allocator(
    void *(*_malloc)(size_t), void *(*_realloc)(void *, size_t),
    void (*_free)(void *, size_t), size_t elsize, size_t cap, uint64_t seed0,
    uint64_t seed1,
    uint64_t (*hash)(const void *item, uint64_t seed0, uint64_t seed1),
    int (*compare)(const void *a, const void *b, void *udata),
    void (*elfree)(void *item, size_t size), void *udata) {
  _malloc = _malloc ? _malloc : __malloc ? __malloc : NULL;
  _realloc = _realloc ? _realloc : __realloc ? __realloc : NULL;
  _free = _free    ? _free
          : __free ? __free
                   : NULL; // default free must be compatible
  size_t ncap = 16;
  if (cap < ncap) {
    cap = ncap;
  } else {
  ...
  // allocate memory for: hashmap + spare + edata
...
// Free all elements stored in the map (if an element free callback is provided)
...
// hashmap_clear quickly clears the map. When update_cap is true, the map's
// capacity is updated to match the currently allocated number of buckets.
...
static bool resize0(struct hashmap *map, size_t new_cap) {
  ...
  map->free(map2, sizeof(struct hashmap) + map2->bucketsz * 2);
  return true;
}
...
void hashmap_free(struct hashmap *map) {
  ...
  free_elements(map);
  map->free(map->buckets, map->bucketsz * map->nbuckets);
  map->free(map, sizeof(struct hashmap) + map->bucketsz * 2);
}
...
//-----------------------------------------------------------------------------
// MurmurHash3 (Murmur3_86_128) implementation
//-----------------------------------------------------------------------------
...
// (Test code omitted for brevity; note that the xfree function below is
// updated)
//==============================================================================
...
// Our test allocation routines now support sized frees.
// xmalloc remains unchanged.
...
// xfree now accepts a size parameter but ignores it since the allocation
// stores its own size.
...
static void free_str(void *item, size_t size) {
  (void)size;
  xfree(*(char **)item, 0);
}
...
static void all(void) {
  ...
  assert(hashmap_xxhash3("hello", 5, 1, 2) == 2584346877953614258ULL);
