...
use crate::{
    HHDM_REQUEST, early_init_pagemap,
    memory::vmm::VMMFlags,
    println, status,
...
impl hpet {
    fn new(hpet_table: acpi_hpet) -> Self {
        ...
        println!(
            "hpet init revision {}, 64 bit or 32 bit counter {}, counter enabled!",
            rev_id, hm
        );
        return hpe;
    }
...
impl CalibrationTimer for hpet {
    fn get_ms(&self) -> usize {
        ((self.read_main_counter() as u128 * self.ctr_clk_period as u128) / 1_000_000_000_000)
            as usize
    }
