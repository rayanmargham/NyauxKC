...
void write_cmos_reg(uint8_t reg, uint8_t data)
{
	arch_raw_io_write(CMOS_SELECT, (1 << 7) | (reg), 1);
...
void get_time()
{
	...
	uint8_t val = read_cmos_reg(0x32);		// century register
	uint8_t val2 = read_cmos_reg(0x09);
	uint8_t val3 = read_cmos_reg(0x08);
	uint8_t val4 = read_cmos_reg(0x07);
	uint8_t val5 = read_cmos_reg(0x04);
	uint8_t val6 = read_cmos_reg(0x02);
	uint8_t val7 = read_cmos_reg(0x00);
