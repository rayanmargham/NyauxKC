...
void init_gdt() {
  ...
  uint8_t access_byte = 0x89; // stolen from osdev wiki cause im lazy to
                              // construct my own basically what it means is it
                              // describes the tss as avabile and stuff
  uint16_t sizeoftss = sizeof(struct TSS) - 1;
  uint64_t constructedlow =
      ((uint64_t)(base_low & 0xffff) << 16) | ((uint64_t)base_mid << 32) |
