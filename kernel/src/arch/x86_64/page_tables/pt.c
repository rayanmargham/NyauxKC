...
uint64_t* find_pte_and_allocate(uint64_t* pt, uint64_t virt)
{
	...
	for (int i = 0; i < 4; i++)
	{
		...
		uint64_t* page_table = (uint64_t*)((uint64_t)pt + hhdm_request.response->offset);
		if (i == 3)
		{
		...
		{
			uint64_t* guy = (uint64_t*)((uint64_t)pmm_alloc() - hhdm_request.response->offset);
...
void x86_64_map_vmm_region(pagemap* take, uint64_t base, uint64_t length_in_bytes)
{
	uint64_t length_in_4kib = length_in_bytes / PAGESIZE;
	for (uint64_t i = 0; i != length_in_4kib; i++)
	{
		void* page = (uint64_t*)((uint64_t)pmm_alloc() - hhdm_request.response->offset);
		map(take->root, (uint64_t)page, base + (i * PAGESIZE), PRESENT | RWALLOWED);
	}
}
void x86_64_unmap_vmm_region(pagemap* take, uint64_t base, uint64_t length_in_bytes)
{
...
uint64_t x86_64_map_kernelhhdmandmemorymap(pagemap* take)
{
	...
	for (uint64_t i = 0; i < 0x100000000; i += MIB(2))
	{
		...
		map2mb(take->root, i, hhdm_request.response->offset + i, PRESENT | RWALLOWED);
		hhdm_pages += 1;
	}
		...
		switch (entry->type)
		{
			case LIMINE_MEMMAP_FRAMEBUFFER:
			{
				...
				while (length >= PAGESIZE)
				{
					...
					{
						map2mb(take->root, entry->base + length, hhdm_request.response->offset + entry->base + length,
					...
					{
						map(take->root, entry->base + length, hhdm_request.response->offset + entry->base + length,
							PRESENT | RWALLOWED);
						addr += PAGESIZE;
						length -= PAGESIZE;
					}
...
{
	take->root = (uint64_t*)((uint64_t)pmm_alloc() - hhdm_request.response->offset);
}
