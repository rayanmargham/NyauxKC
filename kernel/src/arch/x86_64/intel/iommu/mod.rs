...
pub fn iommu_init() {
    ...
    if let Ok(table) = find_acpi_table(c"DMAR".as_ptr()) {
        ...
        let mut ph = Vec::new();
        for i in
            (iommu_addr..(iommu_addr + Processor::PAGE_SIZE as u64)).step_by(Processor::PAGE_SIZE)
        {
            ph.push(i);
        }
        ...
        let ecap = unsafe { iommu_virt.cast::<u64>().add(2).read_volatile() };
        println!("ecap {:b}", ecap);
        if ecap & (1 << 6) == 0 {
        ...
        for i in d.iter() {
            let context_table = {
                if root.entries[i.0 as usize] == 0 {
                    ...
                    unsafe {
                        core::ptr::with_exposed_provenance_mut::<context_table>(
        ...
        while (unsafe { gs.read_volatile() & (1 << 30) } == 0) {}
        status = unsafe { gs.read_volatile() & 0x96FFFFFF };
        status = status | (1 << 31);
