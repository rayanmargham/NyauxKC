...
impl PTENT {
    ...
    pub fn get_table(
        ...
        for level in (target_level..levels).rev() {
            ...
            let ble = unsafe { table.add(index) };
            let data = unsafe { ble.read() };
            let perm = PT::new(&PTENT(data));
            if level == target_level
                || perm.intersects(PT::READ | PT::WRITE | PT::EXECUTE) && perm.contains(PT::VALID)
            {
                return Ok(ble);
            }
...
pub fn pt_init() -> (usize, usize) {
    ...
    match mode {
        limine::paging::PagingMode::RISCV_SV39 => {
            println!("we will use sv39");
            unsafe { SV_LEVELS = 3 };
        }
        limine::paging::PagingMode::RISCV_SV48 => {
            println!("we are going to use sv48");
            unsafe {
                SV_LEVELS = 4;
            }
        }
        limine::paging::PagingMode::RISCV_SV57 => {
    ...
    let yo =
        PTENT(allocate_page().expose_provenance() as u64 - HHDM_REQUEST.response().unwrap().offset);
    ...
    for i in (0..max_hhdm_phys).step_by(Processor::PAGE_SIZE) {
        let flags = { PT::GLOBAL | PT::VALID | PT::WRITE | PT::READ | PT::EXECUTE };
        yo.map4kib(
