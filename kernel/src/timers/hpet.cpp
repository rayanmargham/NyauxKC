...
HPET::HPET() {
  ...
  if (st != UACPI_STATUS_OK) {
    panic((
        char
            *)"The Nyaux Kernel does not support Devices without a HPET\r\nThe "
              "Reason for this is becausei hate tsc with a burning desire. pr "
              "in a "
  ...
  volatile uint32_t *cap = (volatile uint32_t *)(hpet->address.address +
                                                 hhdm_request.response->offset);
  if (!(*cap & (1 << 13))) {
    kprintf("init_hpet(): Bit 13 is off\r\ninit_hpet(): Panicking.\r\n");
    bit32 = true;
    volatile uint64_t *capr =
        (volatile uint64_t *)(hpet->address.address +
                              hhdm_request.response->offset);
    ctr_clock_period_fs = ((volatile uint32_t)(*capr >> 32));
    kprintf("init_hpet(): Counter Clock Period in fentoseconds%lu\r\n",
            ctr_clock_period_fs);
    capr = (volatile uint64_t *)(hpet->address.address +
                                 hhdm_request.response->offset + 0x10);
    *capr |= 1; // enable counter
    kprintf("HPET config after enable write: 0x%lx\r\n", *capr);

    kprintf("init_hpet(): Main Counter Enabled!\r\n");
    hpetvirtaddr = (volatile uint64_t *)(hpet->address.address +
                                         hhdm_request.response->offset);
    bit32 = true;
  } else {
    volatile uint64_t *capr =
...
size_t HPET::get_fs() {
  ...
  volatile void *counter_reg =
      (volatile void *)((uintptr_t)hpetvirtaddr + 0xF0);
...
int HPET::stall_poll_ns(size_t ns) {
  ...
  if (!bit32) {
    volatile uint64_t *hpet_timer =
        reinterpret_cast<volatile uint64_t *>(hpetvirtaddr + 0xF0);
    uint64_t pol_start = *hpet_timer;

    while ((*hpet_timer - pol_start) * ctr_clock_period_fs < ns * 1000000) {
    }
  } else {
    volatile uint32_t *hpet_timer =
        reinterpret_cast<volatile uint32_t *>(hpetvirtaddr + 0xF0);
    uint32_t pol_start = *hpet_timer;

    while (static_cast<uint64_t>(*hpet_timer - pol_start) *
               ctr_clock_period_fs <
           ns * 1000000) {
    }
  }
...
int HPET::stall_poll_us(size_t usec) {
  ...
  if (!bit32) {
    ...
    while ((*hpet_timer - pol_start) * ctr_clock_period_fs <
           usec * 1000000000) {
    }
  } else {
    volatile uint32_t *hpet_timer =
...
int HPET::stall_poll_ms(size_t ms) {
  ...
  if (!bit32) {
    volatile uint64_t *hpet_timer = reinterpret_cast<volatile uint64_t *>(
        (uint64_t)hpetvirtaddr + hpet_offset);
    uint64_t pol_start = *hpet_timer;
    ...
    volatile uint32_t *hpet_timer =
        reinterpret_cast<volatile uint32_t *>(hpetvirtaddr + hpet_offset);
    uint32_t pol_start = *hpet_timer;
...
size_t HPET::get_ps() { return 0; }

int HPET::stall_poll_ps(size_t) { return -1; }

int HPET::stall_poll_fs(size_t) { return -1; }
