...
class GenericTimer {
    ...
    virtual size_t get_ps() {
        return 0;
    }; // pico second
    virtual size_t get_fs() {
        return 0;
    } // fento second
    virtual size_t get_ns() {
        return 0;
    } // nanosecond
    virtual size_t get_us() {
        return 0;
    } // microsecond
    virtual size_t get_ms() {
        return 0;
    };
    virtual int stall_poll_ps(size_t ps) {
        return -1;
    }; // returns -1 if this cannot be done
    virtual int stall_poll_fs(size_t fs) {
        return -1;
    };
    virtual int stall_poll_ns(size_t ns) {
        return -1;
    };
    virtual int stall_poll_us(size_t us) {
        return -1;
    };
    virtual int stall_poll_ms(size_t ms) {
        return -1;
    };
...
extern void *Timer; // ignore this warning, pragma once is protecting us from an ODR
#ifdef __cplusplus
extern "C" {
#endif
